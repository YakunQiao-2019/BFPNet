...
class Res2Net_Ours(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000):
        ...
        self.scale = scale
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 32, 3, 2, 1, bias=False),  # 相当于池化，将特征的W和H缩小一半
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 32, 3, 1, 1, bias=False),
            nn.BatchNorm2d(32),
    ...
    def forward(self, x):
        x0 = self.maxpool(self.relu(self.bn1(self.conv1(x))))  # 10 3 256 256
        x1 = self.layer1(x0)
        x2 = self.layer2(x1)
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)
...
def res2net50_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        print("-------------加载res2net50_v1b_26w_4s-3cf99910.pth预训练参数---------------")
        preModel_path = r'/root/sumail/PolypSegmentation/BFP_Net/preTrain/res2net50_v1b_26w_4s-3cf99910.pth'
        model_state = torch.load(preModel_path)
...
def res2net50_v1b_Ours(pretrained=False, **kwargs):
    ...
    if pretrained:
        preModel_path = r'/root/sumail/PolypSegmentation/preTrain/res2net50_v1b_26w_4s-3cf99910.pth'
        model_state = torch.load(preModel_path)
        model.load_state_dict(model_state)
...
def Res2Net_model(ind=50, pretrained=False):
    if ind == 50:
        model_base = res2net50_v1b(pretrained)
...
if __name__ == '__main__':
    ...
    model = Res2Net_Ours(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4).cuda()
